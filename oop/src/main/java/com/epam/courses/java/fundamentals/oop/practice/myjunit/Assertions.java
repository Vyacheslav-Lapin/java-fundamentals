package com.epam.courses.java.fundamentals.oop.practice.myjunit;
...
public class Assertions {
  ...
  @SneakyThrows
  private Stream<Tuple2<Method, Either<Exception, Boolean>>> execTestClass(
      String className) {
    ...
    Stream<Tuple2<Method, Either<Exception, Boolean>>> result =
        Arrays.stream(methods)
            ...
            .peek(method -> method.setAccessible(true))
            .map(method1 -> Tuple.of(method1, initAndRun(method1)));
   ...
   */
  @SneakyThrows
  Either<Exception, Boolean> initAndRun(@NotNull Method method) {
