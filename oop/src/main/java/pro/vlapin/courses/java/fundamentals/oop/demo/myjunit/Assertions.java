...
import static pro.vlapin.courses.java.fundamentals.oop.annotations.AnnotationUtils.isDeepAnnotationPresent;
import static pro.vlapin.courses.java.fundamentals.oop.annotations.AnnotationUtils.isPseudoNullValue;
...
import pro.vlapin.courses.java.fundamentals.oop.annotations.AnnotationUtils;
...
public class Assertions {
  ...
  private void printResult(@NotNull Tuple2<Method, Either<Exception, Boolean>> tuple2) {
    AnnotationUtils.getDeepAnnotation(tuple2._1, Test.class).ifPresent(
        test -> log.info("Test {} has {}",
                         AnnotationUtils.isPseudoNullValue(test.value()) ? tuple2._1.getName() : test.value(),
                         tuple2._2.isRight() ?
  ...
  @SneakyThrows
  private Stream<Tuple2<Method, Either<Exception, Boolean>>> execTestClass(String className) {
    ...
    Arrays.stream(methods)
        ...
        .filter(staticMethod -> AnnotationUtils.isDeepAnnotationPresent(staticMethod, BeforeAll.class))
    ...
    Stream<Tuple2<Method, Either<Exception, Boolean>>> result =
        Arrays.stream(methods)
            .filter(method -> !Modifier.isStatic(method.getModifiers()))
            .filter(dynamicMethod -> AnnotationUtils.isDeepAnnotationPresent(dynamicMethod, Test.class))
    ...
    Arrays.stream(methods)
        .filter(method -> Modifier.isStatic(method.getModifiers()))
        .filter(staticMethod -> AnnotationUtils.isDeepAnnotationPresent(staticMethod, AfterAll.class))
  ...
  static Either<Exception, Boolean> initAndRun(@NotNull Method method) {
    ...
    Arrays.stream(methods)
        ...
        .filter(dynamicMethod -> AnnotationUtils.isDeepAnnotationPresent(dynamicMethod, Before.class))
    ...
    Arrays.stream(methods)
        .filter(method1 -> !Modifier.isStatic(method1.getModifiers()))
        .filter(dynamicMethod -> AnnotationUtils.isDeepAnnotationPresent(dynamicMethod, After.class))
        .map(beforeMethod -> CheckedFunction1.narrow(beforeMethod::invoke).unchecked())
        .forEach(beforeMethod -> beforeMethod.apply(instance));
